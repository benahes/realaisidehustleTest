export const metadata = {
    title: 'Settings | AI Intelligence',
    description: 'System Preferences and Control Center',
};

const API_TOKENS = [
    {
        name: "production_core_ai_01",
        level: "Full Read/Write",
        levelClass: "bg-secondary-container text-secondary",
        lastUsed: "2 mins ago",
        rowClass: "",
    },
    {
        ...
        level: "Read Only",
        levelClass: "bg-surface-container-highest text-on-surface-variant",
        lastUsed: "14 hours ago",
        rowClass: "",
    },
    {
        name: "test_sandbox_v2",
        level: "Full Read/Write",
        levelClass: "bg-secondary-container text-secondary",
        lastUsed: "Never",
        rowClass: "bg-surface-container-highest/10",
    },
];
        ...
        <main className="max-w-container-max mx-auto px-3 sm:px-margin-edge pb-3 sm:pb-5 pt-[10px] sm:pt-[15px]">
            <div className="flex flex-col md:flex-row gap-2 sm:gap-gutter">
                ...
                <div className="flex-grow space-y-3 sm:space-y-5 relative">
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-surface-container-highest/30 border-b border-outline-variant/20">
                                        ...
                                        <th className="px-2.5 sm:px-6 py-2 sm:py-3 font-label-caps text-xs sm:text-[13px] text-on-surface-variant tracking-widest uppercase text-right">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {API_TOKENS.map((token, i) => (
                                        <tr key={i} className={`border-b border-outline-variant/10 hover:bg-surface-container-highest/20 transition-colors ${token.rowClass}`}>
                                            <td className="px-2.5 sm:px-6 py-2 sm:py-3.5 font-mono-data text-xs sm:text-[14px]">{token.name}</td>
                                            <td className="px-2.5 sm:px-6 py-2 sm:py-3.5">
                                                <span className={`px-1.5 sm:px-2.5 py-0.5 text-xs sm:text-[13px] rounded font-medium ${token.levelClass}`}>
                                                    {token.level}
                                                </span>
                                            </td>
                                            <td className="px-2.5 sm:px-6 py-2 sm:py-3.5 text-xs sm:text-[14px] text-on-surface-variant hidden sm:table-cell">{token.lastUsed}</td>
                                            <td className="px-2.5 sm:px-6 py-2 sm:py-3.5 text-right">
                                                <button className="text-outline hover:text-error transition-colors">
                                                    <span className="material-symbols-outlined text-base sm:text-[20px]">delete</span>
                                                </button>
                                            </td>
                                        </tr>
                                    ))}
                    ...
                    <section className="glass-panel rounded-xl border border-outline-variant/20 p-3 sm:p-6" id="interface">
                        <h2 className="font-h1 text-lg sm:text-[24px] mb-3 sm:mb-5">Interface Customization</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-gutter">
                            {/* Density Toggle */}
                            <div className="p-2.5 sm:p-4 border border-outline-variant/20 rounded-lg bg-surface-container-low space-y-2.5 sm:space-y-4">
                                <div className="flex justify-between items-center">
                                    <span className="font-label-caps text-xs sm:text-[13px] text-on-surface-variant uppercase tracking-widest">UI Density</span>
                                    <span className="material-symbols-outlined text-primary text-base sm:text-[20px]">density_medium</span>
                                </div>
                                <div className="flex p-1 bg-surface-container-highest rounded-sm">
                                    <button className="flex-1 py-1 sm:py-1.5 text-xs sm:text-[13px] font-label-caps text-center hover:text-primary transition-colors uppercase">LOOSE</button>
                                    <button className="flex-1 py-1 sm:py-1.5 text-xs sm:text-[13px] font-label-caps text-center bg-primary text-on-primary rounded-sm uppercase">TIGHT</button>
                                    <button className="flex-1 py-1 sm:py-1.5 text-xs sm:text-[13px] font-label-caps text-center hover:text-primary transition-colors uppercase">XL</button>
                                </div>
                            </div>
                            ...
                            <div className="p-2.5 sm:p-4 border border-outline-variant/20 rounded-lg bg-surface-container-low space-y-2.5 sm:space-y-4">
                                <div className="flex justify-between items-center">
                                    <span className="font-label-caps text-xs sm:text-[13px] text-on-surface-variant uppercase tracking-widest">Font Scaling</span>
                                    <span className="material-symbols-outlined text-primary text-base sm:text-[20px]">format_size</span>
                                </div>
                                <input className="w-full h-1 sm:h-1.5 bg-surface-container-highest rounded-lg appearance-none cursor-pointer accent-primary" max="120" min="80" type="range" defaultValue="100" />
                                <div className="flex justify-between text-xs sm:text-[13px] opacity-50">
                                    <span>80%</span>
                                    <span>100%</span>
                                    <span>120%</span>
                                </div>
                            ...
                            {/* Animations */}
                            <div className="p-2.5 sm:p-4 border border-outline-variant/20 rounded-lg bg-surface-container-low flex items-center justify-between">
                                <div>
                                    <span className="font-label-caps text-xs sm:text-[13px] text-on-surface-variant block mb-1 uppercase tracking-widest">Motion Reduced</span>
                                    <span className="text-xs sm:text-[13px] text-on-surface-variant">Disable micro-interactions</span>
                                </div>
                                <div className="w-10 h-5 bg-surface-container-highest rounded-full p-1 relative cursor-pointer group">
                                    <div className="w-3 h-3 bg-outline rounded-full transition-transform group-hover:scale-110"></div>
                                </div>
                            </div>
                        </div>
                    </section>
                    ...
                    <div className="sticky bottom-3 sm:bottom-5 glass-panel border border-primary/30 rounded-full px-3 sm:px-6 py-2 sm:py-3 flex justify-between items-center shadow-[0_10px_30px_rgba(0,0,0,0.5)] z-20">
                        <div className="hidden sm:flex items-center gap-2 text-on-surface-variant">
                            <span className="material-symbols-outlined text-base sm:text-[18px]">info</span>
                            <span className="text-xs sm:text-[14px]">You have unsaved configuration changes.</span>
                        </div>
                        <div className="flex gap-2 sm:gap-3 w-full sm:w-auto justify-end">
                            <button className="px-3 sm:px-6 py-1 sm:py-1.5 text-xs sm:text-[13px] font-label-caps hover:text-primary transition-colors uppercase">DISCARD</button>
